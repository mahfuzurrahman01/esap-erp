...
import { ReturnProcessApprovalQueryOptions } from "@/modules/scm/types/logistics-and-transport/return-process/return-process-approval-types"

import { approvalStatusOptions } from "./status-option"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ReturnProcessApprovalQueryOptions
  updateParams?: (newParams: Partial<ReturnProcessApprovalQueryOptions>) => void
  onClear?: () => void
}
  ...
  return (
    <>
      ...
      <DateFiled
        ...
        selected={params?.approvalDate ? new Date(params.approvalDate) : null}
        onChange={(e: any) => {
          if (e) {
            // Format date to YYYY-MM-DD
            const formattedDate = e.toISOString().split("T")[0]
            updateParams?.({ approvalDate: formattedDate })
          } else {
            updateParams?.({ approvalDate: undefined })
          }
        }}
        autoComplete="off"
      />
...
export default function ReturnProcessApprovalTableToolbar<
