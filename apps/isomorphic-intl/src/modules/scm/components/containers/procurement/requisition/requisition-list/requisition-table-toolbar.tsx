...
import { RequisitionQueryOptions } from "@/modules/scm/types/procurement/requisition/requisition-types"

import { billingStatusOptions, priorityStatusOptions } from "./status-options"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: RequisitionQueryOptions
  updateParams?: (newParams: Partial<RequisitionQueryOptions>) => void
  onClear?: () => void
}
  ...
  return (
    <>
      ...
      <Select
        ...
        placeholder={t("table-text-priority")}
        className="min-w-[242px]"
      />
      <Input
        type="search"
        className="min-w-[242px]"
        placeholder={t("table-text-requested-by")}
        value={params?.requestedBy}
        onClear={() => updateParams?.({ requestedBy: "" })}
        onChange={(e) => updateParams?.({ requestedBy: e.target.value })}
