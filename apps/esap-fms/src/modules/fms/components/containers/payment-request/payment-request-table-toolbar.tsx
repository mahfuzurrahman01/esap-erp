...
import { PaymentRequestQueryOptions } from "@/modules/fms/types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: PaymentRequestQueryOptions
  updateParams?: (newParams: Partial<PaymentRequestQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  //   updateParams?.({
  //     paymentRequestId: option?.value || "",
  //     pageIndex: 1,
  //   })
  // }

  // const handlePartyTypeChange = (option: any) => {
  //   updateParams?.({
      ...
      {/* <Select
        ...
        value={
          transactionPaymentTypeOptions.find(
            (option) => option.value === params?.paymentRequestId
          ) || null
        }
        onChange={handlePaymentTypeChange}
        isLoading={isPaymentTypeLoading}
        isDisabled={isPaymentTypeLoading}
        placeholder={
          isPaymentTypeLoading
            ? "Loading payment types..."
            : t("table-text-select-payment-type")
        }
      ...
      <Select
        ...
        value={
          partyTypeOptions.find(
            (option) => option.value === params?.partyId
          ) || null
        }
        onChange={handlePartyTypeChange}
        isLoading={isPartyTypeLoading}
        isDisabled={isPartyTypeLoading}
        placeholder={
          isPartyTypeLoading
            ? "Loading party types..."
            : t("table-text-select-party-type")
        }
        className="min-w-[242px]"
      /> */}
      {isFiltered && (
