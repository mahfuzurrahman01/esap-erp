...
const textareaStyles = tv({
  slots: {
    wrapper: "relative",
    label: "block text-sm font-medium text-gray-900 dark:text-white transition-colors duration-200 mb-2 relative [&.required]:after:content-['*'] [&.required]:after:ml-1 [&.required]:after:text-orange-500",
    textarea:
  ...
  (
    ...
    return (
      <div className={wrapper()}>
        {labelText && (
          <label className={cn(label(), isRequired && "required")}>
            {labelText}
          </label>
        )}
        <RizzTextarea
