...
import { useCarrierById } from "@/modules/scm/hooks/logistic-and-transport/carrier/use-carrier"
import { Carrier } from "@/modules/scm/types/logistics-and-transport/carriers/carriers-types"

export default function CarriersDrawerView({
  initialData,
}: {
  initialData: Carrier
}) {
  ...
  const { data, isLoading } = useCarrierById(Number(initialData?.id))

  // Use useMemo to memoize any derived data if needed
  const memoizedData: Carrier | undefined = useMemo(
    () => data as Carrier,
    [data]
  )
  ...
  return (
    <div className="flex h-full flex-col">
      <DrawerHeader
        heading={t("text-carriers-details")}
        onClose={closeDrawer}
