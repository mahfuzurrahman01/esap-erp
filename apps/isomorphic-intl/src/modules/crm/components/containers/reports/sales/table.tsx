...
import SalesReportTableToolbar from "./sales-report-table-toolbar"

export default function SalesReportTable() {
  const { direction } = useDirection()
  const columns = useColumn()
  const { params, updateParams } = useQueryParams<SalesReportQueryOptions>({
    params: [
      {
        key: "search",
        defaultValue: "",
  ...
  const { data, isLoading, isPending } = useSalesReportList({
  ...
  const { table, sensors, handleDragEndColumn, columnOrder } =
    useTanStackTable<SalesReportList>({
      tableData: data?.data ?? [],
