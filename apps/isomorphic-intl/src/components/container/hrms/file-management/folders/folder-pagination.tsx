...
export default function FolderPagination<TData extends Record<string, any>>({
  table,
  params,
  count,
  updateParams,
  ...
  count: number
  updateParams: (params: Partial<folderQueryOptions>) => void
}) {
  const t = useTranslations("hrms")
  const tTable = useTranslations("table")

  const options = [
    { value: 4, label: `4 ${  t("text-items")}` },
    { value: 8, label: `8 ${  t("text-items")}` },
    { value: 12, label: `12 ${  t("text-items")}` },
    { value: 16, label: `16 ${  t("text-items")}` },
  ]
    ...
    <div className="flex w-full items-center justify-between px-5 @container">
      <div className="hidden @2xl:block">
        <Text className="dark:text-title">
          {showCount} of {count} {t("text-items")}
        </Text>
      </div>
