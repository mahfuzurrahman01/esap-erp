...
import { emailSetting } from "@/modules/crm/service/email-setting.service"
import {
  SettingList,
  SettingPaginator,
  SettingQueryOptions,
...
export const SETTING_KEYS = createQueryKeys("EmailSetting")
...
export function useSettingList(options?: Partial<SettingQueryOptions>) {
  ...
  return useQuery<SettingPaginator, Error>({
    ...
    queryFn: ({ queryKey, pageParam }) => {
      return emailSetting.all(Object.assign({}, queryKey[1], pageParam))
    },
...
export function useCreateSetting() {
  ...
  return useMutation({
    mutationFn: (data: any): Promise<any> => emailSetting.create(data),
...
export function useDeleteSetting() {
  ...
  return useMutation({
    mutationFn: (id: any) => emailSetting.delete(id),
    onMutate: async (id) => {
