...
import { LeadQueryOptions } from "@/modules/crm/types/lead"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: LeadQueryOptions
  updateParams?: (newParams: Partial<LeadQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  return (
    <>
      <Input
        type="text"
        placeholder={t("table-text-title")}
        value={params?.title}
      ...
      <Input
        ...
        value={params?.phone}
        onClear={() => updateParams?.({ phone: "" })}
        onChange={(e) =>
          updateParams?.({ phone: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
...
export default function LeadTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
