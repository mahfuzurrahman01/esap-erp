...
import { ContactQueryOptions } from "@/modules/crm/types/contact"
import { dropdownStyles } from "../meeting/meeting-css"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ContactQueryOptions
  updateParams?: (newParams: Partial<ContactQueryOptions>) => void
  onClear?: () => void
}
  ...
  return (
    <>
      <Select
        ...
        value={
          contactTypeOptions.find((option) => option.label === params?.type) ||
          null
        }
        onChange={handleTypeChange}
        placeholder={t("table-text-select-type")}
...
export default function ContactTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
    ...
    <div className="flex items-start justify-between px-5">
      ...
      <div className="flex shrink-0 grow-0 basis-auto items-center gap-4">
        <Button
          ...
          variant={"outline"}
          className={cn(
            "text-subject h-[36px] rounded-lg border-gray-500/20 text-title pe-3 ps-2.5 dark:text-gray-0",
          ...
          <Popover shadow="sm" placement="bottom-end">
            <Popover.Trigger>
              <ActionIcon
                ...
                className="text-subject h-auto w-auto rounded-lg border-gray-500/20 p-1 px-3 dark:text-gray-0">
