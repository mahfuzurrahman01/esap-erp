...
import { TaskQueryOptions } from "@/modules/crm/types/task"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  updateParams?: (newParams: Partial<TaskQueryOptions>) => void
  onClear?: () => void
  isKanban?: boolean
  isShowingCard?: boolean
  setIsShowingCard?: (value: boolean) => void
}
      ...
      <Select
        ...
        value={
          taskStatusOptions.find(
            (option: any) => option.value == params?.status
          ) || null
        }
...
export default function TaskTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
  isKanban,
  ...
  return (
    <div
      className={cn("flex items-center justify-between", !isKanban && "px-5")}>
      <div className="mr-4 flex flex-wrap items-center gap-4">
        {!isKanban && (
          <Input
            type="search"
      ...
      <div className="flex shrink-0 grow-0 basis-auto items-center gap-4">
        ...
        <Button
          ...
          variant={"outline"}
          className={cn(
            "text-subject h-[36px] rounded-lg border-gray-500/20 pe-3 ps-2.5 text-title dark:text-gray-0",
            !isMediumScreen && showFilters && "border-gray-500/20"
          )}>
          <FilterIcon className="me-1.5 h-[18px] w-[18px]" />
          {!isShowingCard
            ? t("table-text-status-board")
            : t("table-text-table-view")}
        </Button>
        ...
        {table && !isKanban && (
          <Popover shadow="sm" placement="bottom-end">
            ...
            <Popover.Content className="card-shadow dark:text-subject relative z-0 overflow-hidden rounded-lg border-transparent bg-paper before:absolute before:-end-4 before:-top-4 before:size-[80px] before:rounded-full before:bg-blue/50 before:blur-[80px] after:absolute after:-bottom-4 after:-start-4 after:size-[80px] after:rounded-full after:bg-red/50 after:blur-[80px] dark:bg-paper">
