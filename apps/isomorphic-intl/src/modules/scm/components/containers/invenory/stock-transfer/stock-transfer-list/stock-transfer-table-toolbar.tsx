...
import { StockTransferQueryOptions } from "@/modules/scm/types/inventory/stock-transfer/stock-transfer-types"

import { stockTransferStatusOptions } from "./status-option"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: StockTransferQueryOptions
  updateParams?: (newParams: Partial<StockTransferQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.stockTransferNo || params?.status
  )
  ...
  return (
    <>
      <Input
        type="text"
        placeholder={t("table-text-stock-transfer-no")}
        className="min-w-[242px]"
        value={params?.stockTransferNo}
        onClear={() => updateParams?.({ stockTransferNo: "" })}
        onChange={(e) => updateParams?.({ stockTransferNo: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      <Select
        labelClassName="text-title"
        options={stockTransferStatusOptions}
        value={
          stockTransferStatusOptions.find(
