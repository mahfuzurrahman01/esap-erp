...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  updateParams?: (newParams: Partial<ProductQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: categories, isLoading: isCategoryLoading } = useCategoryList()
  const categoryOptions = useSelectOptions<CategoryList>(
    categories?.data,
    "name"
  )
  ...
  return (
    <>
      ...
      <Input
        type="text"
        placeholder={t("table-text-actual-price")}
        value={params?.actualPrice}
        onClear={() => updateParams?.({ actualPrice: "" })}
        onChange={(e) =>
          updateParams?.({ actualPrice: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
      ...
      <Input
        type="number"
        placeholder={t("table-text-sell-price")}
        value={params?.salePrice}
        onClear={() => updateParams?.({ salePrice: "" })}
        onChange={(e) =>
          updateParams?.({ salePrice: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
      ...
      <Select
        ...
        value={
          categoryOptions.find((option) => option.label === params?.category) ||
          null
        }
        ...
        placeholder={
          isCategoryLoading
            ? "Loading Category..."
            : t("table-text-select-category")
        }
        className="min-w-[242px]"
      />

      <Select
        labelClassName="text-title rizzui-select-options text-[#bfc6ce] font-light"
        options={approvalStatus}
        styles={dropdownStyles}
        value={
          approvalStatus.find((option) => option.label == params?.status) ||
          null
        }
        onChange={handleStatusChange}
        placeholder={t("table-text-select-status")}
        containerClassName="rizzui-select-options"
        className="rizzui-select-options min-w-[242px]"
      />
...
export default function ProductTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
