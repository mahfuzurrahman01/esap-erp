...
import { approvalListFilterStatus, approvalTypeOptions } from "@/data/crm/common-column"
import { ApprovalQueryOptions } from "@/modules/crm/types/approval"

import { dropdownStyles } from "../meeting/meeting-css"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ApprovalQueryOptions
  updateParams?: (newParams: Partial<ApprovalQueryOptions>) => void
  onClear?: () => void
}
      ...
      <Select
        ...
        value={
          approvalTypeOptions.find(
            (option: any) => option.value == params?.type
          ) || null
        }
        onChange={handleTypeChange}
        placeholder={t("table-text-select-approval-type")}
        className="min-w-[242px]"
      />

      <Select
        labelClassName="text-title"
        options={approvalListFilterStatus}
        styles={dropdownStyles}
        value={
          approvalListFilterStatus.find(
