...
import { JournalDetail, JournalEntryList, JournalTemplateDetail } from "@/modules/fms/types"
import {
...
export default function CreateEditForm({
  ...
  const { coaOptions, isCOALoading, searchCOA } = coa
  ...
  useEffect(() => {
    if (selectedTemplate) {
      // If the template has accounts, use them to create journal entry rows
      ...
      // Fallback to chartOfAccountIds if accounts are not available
      else if (selectedTemplate.chartOfAccountIds) {
        const templateEntries = selectedTemplate.chartOfAccountIds.map(
  ...
  useEffect(() => {
    if (mode !== "create" && journalEntry) {
      if (journalEntry.journalDetails && journalEntry.journalDetails.length > 0) {
        // Map journal details to entries format
        const journalDetails = journalEntry.journalDetails.map(detail => ({
          id: detail.id || 0,
          chartOfAccountId: detail.chartOfAccountId,
          debit: detail.debit || 0,
          credit: detail.credit || 0
        }));
        setEntries(journalDetails);
      }

      if (journalEntry.totalDebit !== undefined && journalEntry.totalDebit !== null) {
        setTotalDebit(Number(journalEntry.totalDebit))
      } else {
        const calculatedDebit = journalEntry.journalDetails.reduce(
          (sum, entry) => sum + (Number(entry.debit) || 0),
          0
        )
      ...
      if (journalEntry.totalCredit !== undefined && journalEntry.totalCredit !== null) {
        setTotalCredit(Number(journalEntry.totalCredit))
      } else {
        const calculatedCredit = journalEntry.journalDetails.reduce(
          (sum, entry) => sum + (Number(entry.credit) || 0),
          0
        )
    ...
    <Box>
      <Form<JournalEntryFormInput>
        ...
        useFormProps={{
          ...
          values: journalEntry ? {
            ...journalEntry,
            // Add journalTemplateId if needed for the form
                    ...
                    render={({ field: { value, onChange } }) => (
                      <Select
                        ...
                        onChange={(option: any) => {
                          ...
                          if (template?.journalTypeId) {
                            setValue?.(
                              "journalTypeId",
                              template.journalTypeId
                            );
                            setValue?.(
                              "companyId",
                              template.companyId
