...
import PriceField from "@/components/base/controlled-table/price-field"
...
import { useJournalEntryTemplateList } from "@/modules/fms/hooks/use-journal-entry-template"
import { useSharedDataHooks } from "@/modules/fms/constants/shared-data-hook"
import { useSelectOptions } from "@/hooks/use-select-options"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: JournalEntryQueryOptions
  updateParams?: (newParams: Partial<JournalEntryQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { journalEntry, company } = useSharedDataHooks(["journalEntry", "company"])
  const { data: journalTemplateList, isLoading: isJournalTemplateLoading } = useJournalEntryTemplateList()

  const { companyOptions = [], isCompanyLoading } = company
  // const { journalEntryTypeOptions = [], isJournalEntryTypeLoading } = journalEntry
  ...
  const isFiltered = Boolean(
    params?.search ||
    params?.journalTypeId ||
    params?.companyId ||
    params?.journalTemplateId ||
    params?.postingDateFrom ||
    params?.postingDateTo ||
    params?.minAmount ||
    params?.maxAmount
  )

  const journalTemplateOptions = useSelectOptions<JournalTemplate>(
    journalTemplateList?.data,
    "journalTemplateTitle"
  );

  const handleJournalEntryTypeChange = (option: any) => {
    updateParams?.({
      journalTypeId: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  const handleJournalTemplateChange = (option: any) => {
    updateParams?.({
      journalTemplateId: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  const handleDateRangeChange = (dates: [Date | null, Date | null]) => {
    ...
    updateParams?.({
      postingDateFrom: start ? start.toISOString() : "",
      postingDateTo: end ? end.toISOString() : "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      {/* <Select
        ...
        value={
          (journalEntryTypeOptions || []).find(
            (option: any) => option.value === params?.journalTypeId
          ) || null
        }
        onChange={handleJournalEntryTypeChange}
        isLoading={isJournalEntryTypeLoading}
        isDisabled={isJournalEntryTypeLoading}
        placeholder={
          isJournalEntryTypeLoading
            ? "Loading journal entry types..."
            : t("table-text-select-journal-entry-type")
        }
      ...
      <Select
        ...
        value={
          (journalTemplateOptions || []).find(
            (option: any) => option.value === params?.journalTemplateId
          ) || null
        }
        onChange={handleJournalTemplateChange}
        isLoading={isJournalTemplateLoading}
        isDisabled={isJournalTemplateLoading}
        placeholder={
          isJournalTemplateLoading
            ? "Loading journal templates..."
            : t("table-text-select-journal-template")
        }
        className="min-w-[242px]"
        menuPortalTarget={document.body}
      />
      ...
      <DateFiled
        ...
        className="w-full"
        placeholderText={t("table-text-select-posting-date")}
        value={
          params?.postingDateFrom ? new Date(params.postingDateFrom) : undefined
        }
        range
        endDate={
          params?.postingDateTo ? new Date(params.postingDateTo) : undefined
        }
        startDate={
          params?.postingDateFrom ? new Date(params.postingDateFrom) : undefined
        }
...
export default function JournalEntryTableToolbar<
