...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: stockLocationList, isLoading: isStockLocationLoading } =
    useStockList({
      pageSize: 1000,
    })
  ...
  const stockLocationOptions = useSelectOptions<Stock>(
    stockLocationList?.data,
    "stockLocation"
  )
  ...
  const handleStockLocationChange = (option: any) => {
    updateParams?.({
      stockLocation: option?.label || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      <Select
        ...
        value={
          productNameOptions.find(
            (option) => option.label === params?.productName
          ) || null
        }
      ...
      <Select
        ...
        value={
          stockLocationOptions.find(
            (option) => option.label === params?.stockLocation
          ) || null
        }
        onChange={handleStockLocationChange}
        isLoading={isStockLocationLoading}
        isDisabled={isStockLocationLoading}
        placeholder={
          isStockLocationLoading
            ? "Loading stock locations..."
            : t("table-text-select-stock-location")
        }
        className="min-w-[242px]"
      />
      ...
      <Input
        type="number"
        placeholder={t("table-text-current-quantity")}
        className="min-w-[242px]"
        value={params?.currentQuantity}
        onClear={() => updateParams?.({ currentQuantity: "" })}
        onChange={(e) => updateParams?.({ currentQuantity: e.target.value })}
...
export default function StockOverviewTableToolbar<
