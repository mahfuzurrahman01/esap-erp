...
export function useBulkDeleteCountry() {
  ...
  return useMutation({
    mutationFn: (ids: number[]) => CountryService.bulk(ids),
    onMutate: async (ids) => {
      await queryClient.cancelQueries({
        queryKey: [COUNTRY_KEYS.all],
        exact: false,
      ...
      const previousCountries = queryClient.getQueryData<CountryPaginator>(queryKey)
    ...
    onSuccess: () => {
      toast.success(<Text as="b">{t("form-country-deleted-successfully")}</Text>)
    },
    onError: (err, variables, context) => {
      // @ts-ignore
      toast.error(<Text as="b">{err.response?.data.message}</Text>)
