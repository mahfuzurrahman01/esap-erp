...
export function useTanStackTable<T extends Record<string, any>>({
  ...
  // If tableData changes, update data only when content actually differs
  // Prevents infinite loop when callers pass data?.data ?? [] (new [] every render)
  React.useEffect(() => {
    setData((prev) => {
      if (tableData === prev) return prev
      if (tableData.length === 0 && prev.length === 0) return prev
      return [...tableData]
    })
  }, [tableData])
