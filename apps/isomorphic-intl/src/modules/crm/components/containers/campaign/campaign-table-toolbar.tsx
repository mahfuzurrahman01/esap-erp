...
import { CampaignQueryOptions } from "@/modules/crm/types/campaign"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: CampaignQueryOptions
  updateParams?: (newParams: Partial<CampaignQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: companies, isLoading: isCompanyLoading } = useCompanyList({pageSize:100})
  ...
  return (
    <>
      <Select
        ...
        value={
          companyOptions.find((option) => option.label === params?.company) ||
          null
        }
      ...
      <Input
        type="text"
        placeholder={t("table-text-service")}
        value={params?.service}
        onClear={() => updateParams?.({ service: "" })}
        onChange={(e) =>
          updateParams?.({ service: e.target.value || "", pageIndex: 1 })
        }
      ...
      <Input
        type="text"
        placeholder={t("table-text-source")}
        value={params?.source}
        onClear={() => updateParams?.({ source: "" })}
        onChange={(e) =>
          updateParams?.({ source: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
      ...
      <Select
        ...
        styles={dropdownStyles}
        value={
          typeOptions.find((option: any) => option.label === params?.type) ||
          null
        }
        onChange={handleTypeChange}
        placeholder={t("table-text-select-campaign-type")}
        className="min-w-[242px]"
      />
