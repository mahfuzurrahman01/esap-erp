...
import { ProductionPlanningReportQueryOptions } from "@/modules/scm/types/feature-reports/production-planning-report"

import { workProgress } from "../../production-control/work-order-tracking/create-work-order-tracking/form-utils"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ProductionPlanningReportQueryOptions
  updateParams?: (
    newParams: Partial<ProductionPlanningReportQueryOptions>
  ) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { workCenter, employee } = useSCMSharedDataHook([
    "workCenter",
    "employee",
  ])
  ...
  const handleAssignToChange = (option: any) => {
    updateParams?.({
      assignTo: option?.label || "",
      pageIndex: 1,
    })
  }

  const handleWorkProgressChange = (option: any) => {
    updateParams?.({
      workProgress: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      {/* <Select
        ...
        value={
          productOptions.find(
            (option: any) => option.value === params?.productName
          ) || null
        }
      ...
      <Select
        ...
        value={
          workCenterOptions.find(
            (option: any) => option.label === params?.workCenter
          ) || null
        }
      ...
      <Select
        ...
        value={
          employeeOptions.find(
            (option: any) => option.label === params?.assignTo
          ) || null
        }
      ...
      <Select
        ...
        value={
          workProgress.find(
            (option: any) => option.value === params?.workProgress
          ) || null
        }
        onChange={handleWorkProgressChange}
        placeholder={t("table-text-select-work-progress")}
        className="min-w-[242px]"
      />
...
export default function ProductionPlanningTableToolbar<
