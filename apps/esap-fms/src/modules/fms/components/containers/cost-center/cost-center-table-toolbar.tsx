...
import { CostCenterQueryOptions } from "@/modules/fms/types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: CostCenterQueryOptions
  updateParams?: (newParams: Partial<CostCenterQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: costCenterList } = useCostCenterList()

  const isFiltered = Boolean(
    params?.search || params?.companyId || params?.status !== ""
  )
  ...
  const statusOptions = useUniqueSelectOptions(
    costCenterList?.data,
    "isDisabled",
    {
      isBooleanField: true,
      booleanLabels: {
        true: t("table-text-inactive"),
        false: t("table-text-active"),
      },
    }
  )
...
export default function CostCenterTableToolbar<
...
>({ table, params, updateParams }: TableToolbarProps<TData>) {
  ...
  const {
    mutateAsync: bulkDeleteCostCenter,
    isSuccess,
    isPending,
  } = useBulkDeleteCostCenter()
  ...
  const bulkDeleteHandle = () => {
    ...
    bulkDeleteCostCenter(selectedIds)
  }
