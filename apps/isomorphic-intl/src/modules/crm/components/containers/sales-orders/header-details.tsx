...
import SalesOrderPdfDocument from "./order-pdf-document"

type salesOrderDetailsHeaderProps = {
  pageHeader: PageHeaderTypes
  salesOrderData: any
  printRef: any
}

export default function SalesOrderDetailsHeader({
  pageHeader,
  salesOrderData,
  printRef,
}: salesOrderDetailsHeaderProps) {
  const tableT = useTranslations("table")
  const reactToPrintContent = () => printRef.current
  const handlePrint = useReactToPrint({
    documentTitle: `SalesOrder_${salesOrderData?.id}`,
  })
  ...
  useEffect(() => {
    if (!salesOrderData || !productList?.data) return
    const productDataMap: { [key: string]: string } = {}
    salesOrderData?.salesOrderDetails?.forEach((item: any) => {
  ...
  }, [salesOrderData, productList])

  const handleDownload = async () => {
    if (!salesOrderData) return
    const salesOrderDetails = salesOrderData?.salesOrderDetails?.map((item: any) => ({
      id: item.productId,
    ...
    try {
      const blob = await pdf(
        <SalesOrderPdfDocument data={salesOrderData} salesOrderDetails={salesOrderDetails} />
      ).toBlob()
      const url = URL.createObjectURL(blob)
      const link = document.createElement("a")
      link.href = url
      link.download = `sales-order-${salesOrderData?.shortOrder}.pdf`
    ...
    <PageHeader title={pageHeader.title} breadcrumb={pageHeader.breadcrumb}>
        <div className="mt-4 flex items-center gap-3 @lg:mt-0">
          <Tooltip
            ...
            color="invert">
            <Link
              href={routes.crm.editSalesOrder(salesOrderData?.id)}
