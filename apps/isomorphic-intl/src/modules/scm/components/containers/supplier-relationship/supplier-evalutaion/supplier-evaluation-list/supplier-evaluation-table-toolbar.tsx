...
import { SupplierEvaluationQueryOptions } from "@/modules/scm/types/supplier-relationship/supplier-evaluation/supplier-evaluation-types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: SupplierEvaluationQueryOptions
  updateParams?: (newParams: Partial<SupplierEvaluationQueryOptions>) => void
  onClear?: () => void
}
  ...
  return (
    <>
      ...
      <Input
        type="text"
        className="min-w-[242px]"
        placeholder={t("table-text-evaluator")}
        value={params?.evaluator}
        onClear={() => updateParams?.({ evaluator: "" })}
        onChange={(e) => updateParams?.({ evaluator: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      <Input
        type="number"
        className="min-w-[242px]"
        placeholder={t("table-text-overall-score")}
        value={params?.overallScore}
        onClear={() => updateParams?.({ overallScore: "" })}
        onChange={(e) => updateParams?.({ overallScore: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
...
export default function SupplierEvaluationTableToolbar<
