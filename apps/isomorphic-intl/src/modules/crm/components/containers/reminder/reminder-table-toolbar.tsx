...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(params?.search || params?.task)
  const { data: tasks, isLoading: isTaskLoading } = useTaskList()
  const taskOptions = useSelectOptions<TaskList>(tasks?.data, "subject")
  ...
  return (
    <>
      {/* <Select
        ...
        options={taskOptions}
        value={
          taskOptions.find((option) => option.label == params?.task) || null
        }
        onChange={handleTaskChange}
        isLoading={isTaskLoading}
        isDisabled={isTaskLoading}
        placeholder={
          isTaskLoading ? t("table-text-loading") : t("table-text-select-task")
        }
...
export default function ReminderTableToolbar<
