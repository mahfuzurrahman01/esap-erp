...
function isActiveRoute(menuPath: string, currentPath: string): boolean {
  ...
  // Get the path segments for both paths
  const menuPathSegments = cleanMenuPath.split('/')
  const currentPathSegments = cleanCurrentPath.split('/')
  
  const lastMenuSegment = menuPathSegments[menuPathSegments.length - 1]
  const lastCurrentSegment = currentPathSegments[currentPathSegments.length - 1]
  
  // For exact path matching (including all segments)
  if (cleanMenuPath === cleanCurrentPath) {
    return true
  }
  
  // Handle nested routes - check if the current path starts with the menu path
  // This ensures parent routes are active when child routes are visited
  if (cleanCurrentPath.startsWith(cleanMenuPath + '/')) {
    return true
  }
  
  // Special case for numeric IDs at the end of paths (e.g., /users/123)
  if (/^\d+$/.test(lastCurrentSegment)) {
    // For routes with IDs, compare all segments except the last one
    const menuPathWithoutLast = menuPathSegments.join('/')
    const currentPathWithoutLast = currentPathSegments.slice(0, -1).join('/')
    
    // Check if the current path without the ID matches the menu path
    if (menuPathWithoutLast === currentPathWithoutLast) {
      return true
    }
    
    // For single segment menu paths (e.g., "users"), match with the first segment of current path
    if (menuPathSegments.length === 1 && menuPathSegments[0] === currentPathSegments[0]) {
      return true
    }
  }
  
  // For matching based on unique last segment (if it's not a number and not already matched)
  // This ensures routes with the same final identifier are considered the same
  if (lastMenuSegment === lastCurrentSegment && lastMenuSegment !== '') {
    // Only match on last segment if the path structure is similar
    // This prevents unrelated paths with same last segment from matching
    if (menuPathSegments.length === currentPathSegments.length) {
      return true
    }
  }
...
function LinkMenuItem({ item, role }: { item: ItemType; role: string[] }) {
  ...
  // Check if current path matches the menu item's path or is a child route
  // const isActive = item.href === "/"
  //   ? normalizedPathname === "/"
  //   : item.href && normalizedPathname.startsWith(item.href)

  const isActive = item.href ? isActiveRoute(item.href, normalizedPathname) : false
...
function CollapsibleMenuItem({
  ...
  const isActive = (() => {
    // Check parent href
    if (item.href && isActiveRoute(item.href, normalizedPathname)) {
      return true
    }

    // Check all submenu items
    return item.subMenuItems?.some(subItem =>
      subItem.href && isActiveRoute(subItem.href, normalizedPathname)
    ) ?? false
  })()

  // Check if dropdown should be open based on current path
  ...
  return (
    <Collapse
      ...
      {item?.subMenuItems?.map((subMenuItem, index) => {
        const isAuthorized = getIsAuthorized(subMenuItem)
        // Use the same isActiveRoute function for consistency
        const isChildActive = subMenuItem.href
          ? isActiveRoute(subMenuItem.href, normalizedPathname)
          : false
