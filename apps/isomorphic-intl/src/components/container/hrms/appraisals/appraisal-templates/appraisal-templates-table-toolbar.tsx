...
import { useBulkDeleteAppraisalTemplates } from "@/hooks/hrms/appraisal/use-appraisal-templates"
import { AppraisalTemplateQueryOptions } from "@/types/hrms/appraisal/appraisal-templates.types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: AppraisalTemplateQueryOptions
  updateParams?: (newParams: Partial<AppraisalTemplateQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(params?.search || params?.departmentId)
...
export default function AppraisalTemplatesTableToolbar<
...
>({ table, params, updateParams }: TableToolbarProps<TData>) {
  ...
  const {
    mutate: bulkDeleteAppraisalTemplate,
    isPending: isBulkDeletePending,
    isSuccess: isBulkDeleteSuccess,
  } = useBulkDeleteAppraisalTemplates()
  // bulk delete handler
  const bulkDeleteHandle = () => {
    const selectedRows = table.getSelectedRowModel().rows
    const selectedIds = selectedRows.map((row) => row.original.id)
    bulkDeleteAppraisalTemplate(selectedIds)
  }
