...
export default function ProfileDrawerView({ user }: { user: User }) {
  ...
  const { data: userData } = useUserById(user?.id) as {
    ...
    <>
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="mt-8 flex flex-col items-center border-b border-dashed px-6 py-8 border-gray-300 dark:border-gray-700">
          <Avatar
            src={
              userData?.data?.profilePicturePath || user?.image || manager.src
            }
          ...
          <div className="my-4 text-center">
            <Title
              as="h6"
              className="text-lg font-semibold text-gray-800 pt-4 pb-2"
            >
        ...
        <div className="border-t px-6 py-4 border-gray-300 dark:border-gray-700 border-dashed">
          <Button
            className="w-full rounded-md bg-red-100 py-6 text-sm font-bold text-red-600 dark:text-red-200 hover:bg-red-300 dark:bg-[#402d2d] hover:dark:bg-[#6c362d]"
            onClick={() => signOut()}
          >
            {t("text-sign-out")}
          </Button>
