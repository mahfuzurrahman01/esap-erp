            ...
            <div className="flex flex-col gap-4 @xl:gap-9">
              ...
              <div className="text-4xl font-bold leading-tight text-title @xl:text-5xl @3xl:text-[64px]">
                {data && (
                  <>
                    {
                      formatCurrency(data?.totalIncome?.totalIncome || 0)
                        .mainPart
                    }
                ...
                <div>
                  {data?.topCompanies?.map((company, index) => (
                    <Avatar
                      ...
                      src={company.logoUrl}
                      className={`relative inline-flex !size-8 object-cover @5xl:!size-10 ${index > 0 ? `-translate-x-[${15 + (index - 1) * 5}px] ring-2 ring-background` : ''
                        }`}
                    />
                  ))}
                </div>
