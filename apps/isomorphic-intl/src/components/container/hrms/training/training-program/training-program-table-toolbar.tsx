...
import { useBulkDeletePrograms } from "@/hooks/hrms/training/program/use-program"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: TrainingProgramQueryOptions
  updateParams?: (newParams: Partial<TrainingProgramQueryOptions>) => void
  onClear?: () => void
}

export default function TrainingProgramTableToolbar<
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.companyName || params?.coordinatorId
  )

  const { employeeOptions, isLoading: isEmployeeLoading } = useEmployeeOptions()

  const handleCoordinatorId = (option: any) => {
    updateParams?.({
      coordinatorId: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      <Select
        ...
        value={
          employeeOptions.find(
            (option) => option.value === params?.coordinatorId
          ) || null
        }
        onChange={handleCoordinatorId}
        isLoading={isEmployeesLoading}
        isDisabled={isEmployeesLoading}
        placeholder={
          isEmployeesLoading
            ? tForm("form-loading-coordinator")
            : tForm("form-select-coordinator")
        }
      ...
      <Select
        ...
        value={
          companyOptions.find(
            (option) => option.label === params?.companyName
          ) || null
        }
        onChange={handleCompany}
        isLoading={isCompaniesLoading}
        isDisabled={isCompaniesLoading}
        placeholder={
          isCompaniesLoading
            ? tForm("form-loading-company")
            : tForm("form-select-company")
        }
        className="min-w-[242px]"
      />
      {isFiltered && (
