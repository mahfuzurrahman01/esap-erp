"use client"

import WidgetCard from "@core/components/cards/widget-card"
import { useDirection } from "@core/hooks/use-direction"
import { DndContext, closestCenter } from "@dnd-kit/core"
import { restrictToHorizontalAxis } from "@dnd-kit/modifiers"

import ApiTablePaginationScm from "@/components/base/api-table-pagination-scm"
import MainTable from "@/components/base/table/main-table"
import {
  DragAbleCellWrapper,
  DragAbleHeadWrapper,
} from "@/components/container/tan-table/custom-table-components"
import { useTanStackTable } from "@/components/container/tan-table/custom-table-components/use-tanstack-table"
import { useQueryParams } from "@/hooks/use-query-params"
import {
  useBulkDeleteSupplier,
  useDeleteSupplier,
  useSupplierList,
} from "@/modules/scm/hooks/procurement/supplier/use-supplier"
import { useTableDelete } from "@/modules/scm/hooks/use-table-delete"
import {
  Supplier,
  SupplierQueryOptions,
} from "@/modules/scm/types/procurement/supplier/supplier-types"

import SupplierTableToolbar from "./supplier-table-toolbar"
import { useSupplierColumn } from "./use-column"

export default function SupplierTable() {
  const columns = useSupplierColumn()
  const { direction } = useDirection()

  const deleteSupplier = useDeleteSupplier()
  const bulkDeleteSupplier = useBulkDeleteSupplier()

  const { params, updateParams } = useQueryParams<SupplierQueryOptions>({
    params: [
      {
        key: "search",
        defaultValue: "",
        parse: (value) => value || "",
      },
      {
        key: "pageIndex",
        defaultValue: 1,
        parse: (value) => Number(value) || 1,
      },
      {
        key: "pageSize",
        defaultValue: 10,
        parse: (value) => Number(value) || 10,
      },
      {
        key: "supplierName",
        defaultValue: "",
        parse: (label) => label || "",
      },
      {
        key: "countryName",
        defaultValue: "",
        parse: (label) => label || "",
      },
      {
        key: "supplierType",
        defaultValue: "",
        parse: (label) => label || "",
      },
      {
        key: "contactNumber",
        defaultValue: "",
        parse: (value) => value || "",
      },
    ],
  })

  const { data, isLoading } = useSupplierList({
    search: params.search,
    pageIndex: params.pageIndex,
    pageSize: params.pageSize,
    supplierName: params.supplierName,
    countryName: params.countryName,
    supplierType: params.supplierType,
    contactNumber: params.contactNumber,
  })

  const { table, setData, sensors, handleDragEndColumn, columnOrder } =
    useTanStackTable<Supplier>({
      tableData: data?.data ?? [],
      columnConfig: columns,
      options: {
        initialState: {
          columnPinning: {
            left: ["id"],
            right: ["action"],
          },
        },
        enableRowSelection: true,
        columnResizeDirection: direction as any,
        columnResizeMode: "onChange",
        enableColumnResizing: true,
        onStateChange: (updater) => {
          if ("data" in updater) {
            table.resetRowSelection()
          }
        },
      },
    })

  const { handleDeleteRow, handleMultipleDelete } = useTableDelete<Supplier>({
    deleteMutation: deleteSupplier.mutate,
    bulkDeleteMutation: bulkDeleteSupplier.mutate,
    setData,
    resetRowSelection: table.resetRowSelection,
  })

  // Update table options with delete handlers
  table.options.meta = {
    ...table.options.meta,
    handleDeleteRow,
    handleMultipleDelete,
  }

  return (
    <>
      <WidgetCard
        rounded="xl"
        className="card-shadow flex flex-col gap-4 border-none bg-paper dark:bg-paper">
        <SupplierTableToolbar
          table={table}
          params={params}
          updateParams={updateParams}
        />
        <DndContext
          collisionDetection={closestCenter}
          modifiers={[restrictToHorizontalAxis]}
          onDragEnd={handleDragEndColumn}
          sensors={sensors}>
          <MainTable
            table={table}
            columnOrder={columnOrder}
            variant={"modern"}
            isLoading={
              isLoading ||
              deleteSupplier.isPending ||
              bulkDeleteSupplier.isPending
            }
            components={{
              headerCell: DragAbleHeadWrapper,
              bodyCell: DragAbleCellWrapper,
            }}
          />
        </DndContext>
        <ApiTablePaginationScm
          table={table}
          params={params}
          count={data?.count || 0}
          updateParams={updateParams}
        />
      </WidgetCard>
    </>
  )
}
