...
import { QuotationQueryOptions } from "@/modules/crm/types/quotation"

import { dropdownStyles } from "../meeting/meeting-css"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: QuotationQueryOptions
  updateParams?: (newParams: Partial<QuotationQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  return (
    <>
      <Input
        type="text"
        placeholder={t("table-text-quotation-no")}
        value={params?.quotationNo}
        onClear={() => updateParams?.({ quotationNo: "" })}
        onChange={(e) =>
          updateParams?.({ quotationNo: e.target.value || "", pageIndex: 1 })
        }
      ...
      <Select
        ...
        value={
          quotationTypes.find((option) => option.label === params?.type) || null
        }
        onChange={handleTypeChange}
        placeholder={t("table-text-select-quote-type")}
        className="min-w-[242px]"
      />
      ...
      <Select
        ...
        value={
          approvalStatus.find(
            (option) => option.label == params?.approvalStatus
          ) || null
        }
