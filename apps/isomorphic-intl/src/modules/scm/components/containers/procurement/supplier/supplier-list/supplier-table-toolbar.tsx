...
import { SupplierQueryOptions } from "@/modules/scm/types/procurement/supplier/supplier-types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: SupplierQueryOptions
  updateParams?: (newParams: Partial<SupplierQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: supplierCategoryList, isLoading: isSupplierCategoryLoading } =
  ...
  const countryNameOptions = useSelectOptions<CountryList>(
    countryList?.data,
    "countryName"
  )
  const supplierCategoryOptions = useSelectOptions<SupplierCategory>(
    supplierCategoryList?.data,
    "name"
  )
  ...
  const handleSupplierCategoryChange = (option: any) => {
    updateParams?.({
      supplierType: option?.label || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      <Input
        ...
        placeholder={t("table-text-supplier-name")}
        value={params?.supplierName}
        onClear={() => updateParams?.({ supplierName: "" })}
        onChange={(e) => updateParams?.({ supplierName: e.target.value })}
      ...
      <Select
        ...
        value={
          countryNameOptions.find(
            (option) => option.label === params?.countryName
          ) || null
        }
      ...
      <Select
        ...
        value={
          supplierCategoryOptions.find(
            (option) => option.label === params?.supplierType
          ) || null
        }
        onChange={handleSupplierCategoryChange}
        isLoading={isSupplierCategoryLoading}
        isDisabled={isSupplierCategoryLoading}
        placeholder={
          isSupplierCategoryLoading
            ? "Loading categories..."
            : t("table-text-select-supplier-type")
        }
        className="min-w-[242px]"
      />
      ...
      <Input
        type="number"
        className="min-w-[242px]"
        placeholder={t("table-text-contact-number")}
        value={params?.contactNumber}
        onClear={() => updateParams?.({ contactNumber: "" })}
        onChange={(e) => updateParams?.({ contactNumber: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
      />
