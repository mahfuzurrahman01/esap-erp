...
export default function PermissionsTable() {
  ...
  const deleteItemBulk = useBulkDeletePermission()
  const columns = useColumn()
  const { params, updateParams } = useQueryParams<PermissionQueryOptions>({
    params: [
      {
        key: "search",
        defaultValue: "",
  ...
  const { data, isLoading, isPending } = usePermissionList({
  ...
  const { table, setData, sensors, columnOrder, handleDragEndColumn } =
    useTanStackTable<PermissionList>({
      tableData: data?.data ?? [],
  ...
  const { handleDeleteRow, handleMultipleDelete } =
    useTableDelete<PermissionList>({
      deleteMutation: deletePermission.mutate,
