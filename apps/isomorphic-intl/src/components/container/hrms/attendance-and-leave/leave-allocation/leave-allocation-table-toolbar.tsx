...
import { useBulkDeleteLeaveAllocation } from "@/hooks/hrms/attendance-and-leave/use-leave-allocation"
...
import { LeaveAllocationQueryOptions } from "@/types/hrms/attendance-and-leave/leave-allocation.types"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: LeaveAllocationQueryOptions
  updateParams?: (newParams: Partial<LeaveAllocationQueryOptions>) => void
  onClear?: () => void
}
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: departmentList, isLoading: isDepartmentLoading } =
    useDepartmentList()
  const { data: leaveTypeList, isLoading: isLeaveTypeLoading } =
    useLeaveTypeList()
  ...
  const handleLeaveTypeChange = (option: any) => {
    updateParams?.({
    ...
    <>
      <Select
        ...
        placeholder={
          isDepartmentLoading
            ? "Loading departments..."
            : t("table-text-select-department")
        }
      ...
      <Select
        ...
        value={
          employeeOptions.find(
            (option) => option.value === params?.managerId
          ) || null
        }
      ...
      <Select
        ...
        value={
          leaveTypeOptions.find(
            (option) => option.value === params?.leaveTypeId
          ) || null
        }
        onChange={handleLeaveTypeChange}
        isLoading={isLeaveTypeLoading}
        isDisabled={isLeaveTypeLoading}
        placeholder={
          isLeaveTypeLoading
            ? "Loading leave types..."
            : t("table-text-select-leave-type")
        }
        className="min-w-[242px]"
      />
...
export default function LeaveAllocationTableToolbar<
...
>({ table, params, updateParams }: TableToolbarProps<TData>) {
  ...
  const {
    mutateAsync: bulkDeleteLeaveAllocation,
    isSuccess: deleteSuccess,
    isPending: isDeletePending,
  } = useBulkDeleteLeaveAllocation()

  const bulkDeleteHandle = () => {
    const selectedRows = table.getSelectedRowModel().rows
    const selectedIds = selectedRows.map((row) => row.original.id)
    bulkDeleteLeaveAllocation(selectedIds)
  }
