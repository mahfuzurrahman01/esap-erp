...
export default function ApiTablePagination<TData extends Record<string, any>>({
  table,
  params,
  count,
  updateParams,
  ...
  const options = [
    ...
    { value: 50, label: t("table-text-rows-per-page-50") },
    // { value: 100, label: t("table-text-rows-per-page-100") },
  ]
  ...
  const handlePageSizeChange = (value: SelectOption) => {
    updateParams({
      ...
      pageIndex: DEFAULT_PAGE_INDEX, // Reset to first page when changing page size
    })
  }
