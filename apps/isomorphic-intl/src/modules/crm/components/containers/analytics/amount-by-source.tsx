...
import { useAmountBySource } from "@/modules/crm/hooks/use-opportunity-analytics"
import { useTranslations } from "next-intl"
import { useTheme } from "next-themes"

export default function AmountBySource() {
  const t = useTranslations("crm")
  const { theme } = useTheme()
  const isDark = theme === "dark"
  const { data: output, isLoading }: any = useAmountBySource() || {}

  const tableData =
    output?.data &&
    output?.data?.map((report: any) => ({
      day: report.leadSource || t("text-unknown"),
