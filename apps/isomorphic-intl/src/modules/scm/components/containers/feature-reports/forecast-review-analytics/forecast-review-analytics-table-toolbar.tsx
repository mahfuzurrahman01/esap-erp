...
import { ForecastReviewAnalyticsQueryOptions } from "@/modules/scm/types/feature-reports/forecast-review-analytics"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ForecastReviewAnalyticsQueryOptions
  updateParams?: (
    newParams: Partial<ForecastReviewAnalyticsQueryOptions>
  ) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { product, salesOperationPlan } = useSCMSharedDataHook([
    "product",
    "salesOperationPlan",
  ])

  const { productOptions, isProductsLoading } = product
  const { salesOperationPlanOptions, isSalesOperationPlanLoading } =
    salesOperationPlan
  ...
  const handleForecastPeriodChange = (option: any) => {
    updateParams?.({
      forecastPeriod: option?.label || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      ...
      <Select
        labelClassName="text-title"
        options={salesOperationPlanOptions}
        value={
          salesOperationPlanOptions.find(
            (option: any) => option.label === params?.forecastPeriod
          ) || null
        }
        onChange={handleForecastPeriodChange}
        isLoading={isSalesOperationPlanLoading}
        isDisabled={isSalesOperationPlanLoading}
        placeholder={
          isSalesOperationPlanLoading
            ? "Loading forecast period..."
            : t("table-text-select-forecast-period")
        }
        className="min-w-[242px]"
      />
...
export default function ForecastReviewAnalyticsTableToolbar<
