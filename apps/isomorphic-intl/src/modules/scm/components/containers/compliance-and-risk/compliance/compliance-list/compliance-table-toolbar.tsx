...
import { ComplianceQueryOptions } from "@/modules/scm/types/compliance-and-risk/compliance-types"
import { Employee } from "@/types/hrms/employee/employee.types"

import { complianceStatusOptions } from "./status-options"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: ComplianceQueryOptions
  updateParams?: (newParams: Partial<ComplianceQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.assignedTo || params?.completionStatus
  )
  ...
  const handleComplianceStatusChange = (option: any) => {
    updateParams?.({
      ...
      <Select
        ...
        value={
          complianceStatusOptions.find(
            (option) => option.value === params?.completionStatus
          ) || null
        }
        onChange={handleComplianceStatusChange}
        placeholder={t("table-text-compliance-status")}
