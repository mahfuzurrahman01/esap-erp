...
import { RoleQueryOptions } from "@/modules/crm/types/role"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: RoleQueryOptions
  updateParams?: (newParams: Partial<RoleQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.name || params?.permission
  )
  ...
  return (
    <>
      ...
      <Select
        labelClassName="text-title rizzui-select-options"
        options={permissionOptions}
        value={
          permissionOptions.find(
            (option) => option.label === params?.permission
          ) || null
        }
        ...
        placeholder={
          isPermissionLoading
            ? "Loading Permissions..."
            : t("table-text-select-permission")
        }
...
export default function RoleTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
