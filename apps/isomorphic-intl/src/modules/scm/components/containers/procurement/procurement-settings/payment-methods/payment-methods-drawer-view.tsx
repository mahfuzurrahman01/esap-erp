...
import { usePaymentMethodById } from "@/modules/scm/hooks/procurement/supplier/use-payment-method"
import { PaymentMethod } from "@/modules/scm/types/procurement/supplier/payment-method-types"

import { getPaymentMethodStatusBadge } from "./status-option"

export default function PaymentMethodsDrawerView({
  initialData,
}: {
  ...
  const { data, isLoading } = usePaymentMethodById(Number(initialData?.id))

  // Use useMemo to memoize any derived data if needed
  const memoizedData: PaymentMethod | undefined = useMemo(
  ...
  return (
    <div className="flex h-full flex-col">
      <DrawerHeader
        heading={t("text-payment-method-details")}
        onClose={closeDrawer}
          ...
          <table className="w-full">
            <tbody>
              ...
              <tr className="gap-4 border-b border-dotted border-gray-500/20 px-5">
                ...
                <td className="py-5">
                  <Text className="ml-3 font-semibold text-gray-900 dark:text-gray-0">
                    {getPaymentMethodStatusBadge(memoizedData?.status ?? false)}
                  </Text>
                </td>
              </tr>
              <tr className="gap-4 border-b border-dotted border-gray-500/20 px-5">
                ...
                <td className="py-5">
                  <Text className="ml-3 font-semibold text-gray-900 dark:text-gray-0">
                    {dayjs(memoizedData?.createdDate || "").format(
                      "YYYY/MM/DD"
                    )}
                  </Text>
                </td>
              ...
              <tr className="gap-4 border-b border-dotted border-gray-500/20 px-5">
                ...
                <td className="py-5">
                  <Text className="ml-3 font-semibold text-gray-900 dark:text-gray-0">
                    {dayjs(memoizedData?.updatedDate || "").format(
                      "YYYY/MM/DD"
                    )}
                  </Text>
                </td>
