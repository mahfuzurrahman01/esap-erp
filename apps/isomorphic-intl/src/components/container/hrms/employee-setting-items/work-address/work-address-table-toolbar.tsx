...
import { useBulkDeleteWorkAddress } from "@/hooks/hrms/employee/use-work-address"
import { useCountryList } from "@/hooks/use-countries"
import { useSelectOptions } from "@/hooks/use-select-options"
import { CountryList } from "@/modules/fms/types"
import { WorkAddressQueryOptions } from "@/types/hrms/employee/work-address.types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: WorkAddressQueryOptions
  updateParams?: (newParams: Partial<WorkAddressQueryOptions>) => void
  onClear?: () => void
}

export default function WorkAddressTableToolbar<
...
>({ table, params, updateParams }: TableToolbarProps<TData>) {
  ...
  const {
    mutateAsync: bulkDeleteWorkAddress,
  ...
  const bulkDeleteHandle = () => {
    ...
    bulkDeleteWorkAddress(selectedIds)
  }
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(params?.search || params?.country)
  ...
  return (
    <>
      <Select
        ...
        value={
          countryOptions.find((option) => option.label === params?.country) ||
          null
        }
        ...
        placeholder={
          isCountryLoading ? "Country..." : t("table-text-select-country")
        }
