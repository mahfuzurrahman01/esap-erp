...
import { useUserByEmail } from "@/hooks/auth/use-user"
...
const UserByEmail = ({ email }: UserByEmailProps) => {
  const tableT = useTranslations("table")
  const {
    data,
    isLoading: isUserLoading,
    error,
  } = useUserByEmail(email!) as {
  ...
  return (
    <AvatarCard
      ...
      name={`${data?.data?.firstName || email || ""} ${data?.data?.lastName || ""}`}
    />
  )
}
