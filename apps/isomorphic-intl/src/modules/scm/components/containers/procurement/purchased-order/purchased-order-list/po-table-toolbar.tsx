...
import { PurchasedOrderQueryOptions } from "@/modules/scm/types/procurement/purchased-order/purchased-order-types"
import { billingStatus } from "@/modules/scm/constants/shared-status-field-option"
import dayjs from "dayjs"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: PurchasedOrderQueryOptions
  updateParams?: (newParams: Partial<PurchasedOrderQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  return (
    <>
      ...
      <DateFiled
        placeholderText={t("table-text-po-date")}
        value={params?.poDate ? dayjs(params.poDate).toDate() : null}
        onChange={(e: any) => updateParams?.({ poDate: e?.toISOString() })}
      ...
      <Input
        type="number"
        className="min-w-[242px]"
        placeholder={t("table-text-order-amount")}
        value={params?.orderAmount}
        onClear={() => updateParams?.({ orderAmount: "" })}
        onChange={(e) => updateParams?.({ orderAmount: e.target.value })}
