...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(params?.search || params?.startDate || params?.endDate)

  return (
    <>
      <DateFiled
        placeholderText={t("table-text-start-date")}
        value={params?.startDate ? dayjs(params.startDate).toDate() : null}
        onChange={(e: any) => updateParams?.({ startDate: e?.toISOString() })}
        className="w-auto min-w-[242px]"
      />
      <DateFiled
        placeholderText={t("table-text-end-date")}
        value={params?.endDate ? dayjs(params.endDate).toDate() : null}
        onChange={(e: any) => updateParams?.({ endDate: e?.toISOString() })}
        className="w-auto min-w-[242px]"
      />
      {isFiltered && (
        <Button
...
export default function CampaignsReportTableToolbar<
