...
import { ForecastQueryOptions } from "@/modules/scm/types/demand-and-forecasting/forecast/forecast-types";
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: ForecastQueryOptions
  updateParams?: (newParams: Partial<ForecastQueryOptions>) => void
  onClear?: () => void
}
  ...
  return (
    <>
      ...
      <Input
        type="text"
        className="min-w-[242px]"
        placeholder={t("table-text-forecast-method")}
        value={params?.forecastMethod}
        onClear={() => updateParams?.({ forecastMethod: "" })}
        onChange={(e) => updateParams?.({ forecastMethod: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />

      <Input
        type="number"
        placeholder={t("table-text-past-sales-data")}
        value={params?.pastSalesData}
        onClear={() => updateParams?.({ pastSalesData: "" })}
        onChange={(e) => updateParams?.({ pastSalesData: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
      />
...
export default function ForecastTableToolbar<
