...
export function useDeleteLeaveAllocation() {
  ...
  const t = useTranslations("form")
  return useMutation({
    mutationFn: (id: number) => LeaveAllocationService.delete(id),
    onMutate: async (id) => {
    ...
    onSuccess: () => {
      toast.success(<Text as="b">{t("form-leave-allocation-deleted")}</Text>)
...
export function useBulkDeleteLeaveAllocation() {
  ...
  const t = useTranslations("form")
  return useMutation({
    mutationFn: (ids: number[]) => LeaveAllocationService.bulkDelete(ids),
    onMutate: async (ids) => {
      await queryClient.cancelQueries({
        queryKey: [LEAVE_ALLOCATION_KEYS.all],
        exact: false,
    ...
    onSuccess: () => {
      toast.success(
        <Text as="b">{t("form-selected-leave-allocations-deleted")}</Text>
      )
    },
