...
import { useUnitById } from "@/modules/scm/hooks/procurement/requisition/use-unit"
import { ItemUnit } from "@/modules/scm/types/procurement/requisition/item-unit-types"

export default function ItemsDrawerView({
  initialData,
}: {
  ...
  const { data, isLoading } = useUnitById(Number(initialData?.id))

  // Use useMemo to memoize any derived data if needed
  const memoizedData: ItemUnit | undefined = useMemo(
    () => data as ItemUnit,
    [data]
  )
