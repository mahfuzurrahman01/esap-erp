...
const selectStyles = tv({
  slots: {
    ...
    menu: [
      "absolute !z-50 w-full min-w-[8rem] max-h-40 rounded-md border border-transparent",
  ...
  (
    ...
    return (
      <div className={wrapper({ class: className })}>
        {label && (
          <label className={cn(labelStyle(), labelClassName)} data-required={isRequired}>
            {label}
          </label>
        )}
          ...
          <ReactSelect
            ...
            components={{
              ...
              DropdownIndicator: isDisabled
                ? () => null
                : () => <CaretDownIcon className="size-3 ml-2" />,
            }}
            styles={{
              menuList: (base: any) => ({
                ...base,
