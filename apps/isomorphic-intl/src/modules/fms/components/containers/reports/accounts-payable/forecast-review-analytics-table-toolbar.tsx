...
import { AccountPayableReportsQueryOptions } from "@/modules/scm/types/feature-reports/account-payable-reports"
import dayjs from "dayjs"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: AccountPayableReportsQueryOptions
  updateParams?: (newParams: Partial<AccountPayableReportsQueryOptions>) => void
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.party || params?.dueDate || params?.payableAccount
  )
  ...
  return (
    <>
      <Input
        type="search"
        placeholder={t("table-text-party")}
        value={params?.party || ""}
        onClear={() => updateParams?.({ party: "" })}
      ...
      <Input
        type="search"
        placeholder={t("table-text-payable-account")}
        value={params?.payableAccount || ""}
        onClear={() => updateParams?.({ payableAccount: "" })}
        onChange={(e) => updateParams?.({ payableAccount: e.target.value })}
        className="min-w-[242px]"
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      ...
      <DateFiled
        ...
        value={params?.dueDate ? dayjs(params.dueDate).toDate() : null}
        onChange={(e: any) => updateParams?.({ dueDate: e?.toISOString() })}
        onClear={() => updateParams?.({ dueDate: "" })}
...
export default function AccountPayableTableToolbar<
