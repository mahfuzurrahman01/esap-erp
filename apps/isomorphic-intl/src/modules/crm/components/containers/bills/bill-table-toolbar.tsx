...
import { billCategoryOptions } from "@/data/crm/campaign"
import { paymentMethods } from "@/data/crm/quotation"
import { BillQueryOptions } from "@/modules/crm/types/bill"

import { dropdownStyles } from "../meeting/meeting-css"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: BillQueryOptions
  updateParams?: (newParams: Partial<BillQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const handleIncomeCategoryChange = (option: any) => {
    updateParams?.({
  ...
  const handlePaymentMethodsChange = (option: any) => {
    updateParams?.({
  ...
  return (
    <>
      <Select
        ...
        value={
          billCategoryOptions.find((option) => option.label === params?.type) ||
          null
        }
        onChange={handleIncomeCategoryChange}
        placeholder={t("table-text-select-category")}
        className="min-w-[242px]"
      />

      <Input
        type="text"
        placeholder={t("table-text-amount")}
        value={params?.amount}
        onClear={() => updateParams?.({ amount: "" })}
        onChange={(e) =>
          updateParams?.({ amount: e.target.value || "", pageIndex: 1 })
        }
      ...
      <Select
        ...
        styles={dropdownStyles}
        value={
          paymentMethods.find((option) => option.label == params?.paymentMethod) || null
        }
        onChange={handlePaymentMethodsChange}
        placeholder={t("table-text-select-payment-method")}
        className="min-w-[242px]"
      />
...
export default function BillTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
