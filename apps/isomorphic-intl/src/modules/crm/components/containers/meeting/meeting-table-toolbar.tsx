...
import { MeetingQueryOptions } from "@/modules/crm/types/meeting"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: MeetingQueryOptions
  updateParams?: (newParams: Partial<MeetingQueryOptions>) => void
  onClear?: () => void
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.title || params?.location
  )
  ...
  return (
    <>
      <Input
        type="text"
        placeholder={t("table-text-enter-title")}
        value={params?.title}
        onClear={() => updateParams?.({ title: "" })}
        onChange={(e) =>
          updateParams?.({ title: e.target.value || "", pageIndex: 1 })
        }
      ...
      <Input
        type="text"
        placeholder={t("table-text-enter-location")}
        value={params?.location}
        onClear={() => updateParams?.({ location: "" })}
        onChange={(e) =>
          updateParams?.({ location: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
...
export default function MeetingTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
  isShowingCard,
