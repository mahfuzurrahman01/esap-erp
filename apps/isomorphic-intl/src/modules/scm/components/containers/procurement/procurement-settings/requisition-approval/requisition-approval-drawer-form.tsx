...
import { requisitionApprovalSchema } from "@/modules/scm/validators/procurement/requisition-approval.schema"

import { approvalStatusOptions } from "./status-option"
import { useRequisitionApprovalForm } from "./use-requisition-approval-form"
import dayjs from "dayjs"

type RequisitionApprovalFormDrawerViewProps = {
  isEditForm?: boolean
  initialData?: RequisitionApproval
} & (
  | { isEditForm: true; initialData: RequisitionApproval }
  | { isEditForm?: false; initialData?: RequisitionApproval }
)

const RequisitionApprovalFormDrawerView = ({
  isEditForm = false,
  initialData,
}: RequisitionApprovalFormDrawerViewProps) => {
  const t = useTranslations("form")

  const { isLoading, onSubmit, handleCloseDrawer } = useRequisitionApprovalForm(
    isEditForm,
    initialData
  )
  ...
  return (
    <div className="flex h-full flex-col">
      <DrawerHeader
        heading={
          isEditForm
            ? t(messages.editRequisitionApproval)
            : t(messages.addNewRequisitionApproval)
        }
        onClose={handleCloseDrawer}
        headerClassName="mb-0"
      />
      <Form<RequisitionApproval>
        validationSchema={requisitionApprovalSchema}
        onSubmit={onSubmit}
        useFormProps={{
          mode: "onChange",
...
export default RequisitionApprovalFormDrawerView
