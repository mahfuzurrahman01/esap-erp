...
import { AssetQueryOptions } from "@/modules/fms/types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: AssetQueryOptions
  updateParams?: (newParams: Partial<AssetQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { assetCategory, assetLocation } = useSharedDataHooks([
    "assetCategory",
    "assetLocation",
  ])
  ...
  const handleAssetStatusChange = (option: any) => {
    updateParams?.({
      assetStatusId: option?.value || "",
      pageIndex: 1,
    })
  }
...
export default function AssetTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
