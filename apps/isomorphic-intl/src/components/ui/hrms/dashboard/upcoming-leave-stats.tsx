...
import { useLeaveRequestList } from "@/hooks/hrms/attendance-and-leave/use-leave-request"
    ...
    <div className="h-[280px] w-full rounded-3xl bg-gradient-to-b from-white via-white/80 to-transparent p-5 shadow-sm backdrop-blur-sm transition-colors duration-200 dark:from-gray-100/10 dark:via-gray-100/5 dark:to-transparent">
      <div className="flex flex-col justify-start gap-4 sm:flex-row">
        {[...Array(3)].map((_, index) => (
          <div
            key={index}
            className="flex w-full flex-col items-center rounded-2xl bg-gray-200/80 p-4 backdrop-blur-sm dark:bg-gray-800/50 dark:backdrop-blur-sm sm:w-1/3">
            {/* Avatar Skeleton */}
            <div className="relative mb-3 h-16 w-16 overflow-hidden rounded-full ring-2 ring-white/10">
              <Skeleton className="h-full w-full bg-gray-300 dark:bg-gray-700/50" />
            </div>

            {/* Name Skeleton */}
            <div className="flex flex-col items-center gap-1">
              <Skeleton className="h-4 w-20 bg-gray-300 dark:bg-gray-700/50" />
              <Skeleton className="h-4 w-24 bg-gray-300 dark:bg-gray-700/50" />
            </div>

            {/* Leave Type Skeleton */}
            <Skeleton className="mt-2 h-6 w-24 rounded-full bg-gray-300 dark:bg-gray-700/50" />

            {/* Status & Duration Skeleton */}
            <div className="mt-3 flex items-center gap-1.5">
              <Skeleton className="h-4 w-4 rounded-full bg-gray-300 dark:bg-gray-700/50" />
              <Skeleton className="h-4 w-16 bg-gray-300 dark:bg-gray-700/50" />
            </div>

            {/* Dates Skeleton */}
            <Skeleton className="mt-2 h-3 w-28 bg-gray-300 dark:bg-gray-700/50" />
          </div>
        ))}
      </div>
    </div>
