...
import { AccountRecivableQueryOptions } from "@/modules/crm/types/account-recivable"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: AccountRecivableQueryOptions
  updateParams?: (newParams: Partial<AccountRecivableQueryOptions>) => void
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.invoiceDateFrom
 || params?.deliveryDateFrom || params?.customerId || params?.courier || params?.status
  )

  const { customer } = useSharedDataHooks([
    "customer",
  ])
  ...
  return (
    <>
      <Select
        ...
        value={
          params?.customerId && customerOptions
            ? customerOptions.find(
              (option: any) => option.value == params?.customerId
            )
            : null
        }
        isLoading={isCustomerLoading}
        isDisabled={isCustomerLoading}
        onChange={(e: any) => updateParams?.({ customerId: e?.value })}
        placeholder={t("table-text-select-customer")}
      />

      <DateFiled
        dateFormat="dd-MMM-yyyy"
        id="fromDate"
        placeholderText={t("table-text-invoice-date")}
        value={params?.invoiceDateFrom
 ? dayjs(params.invoiceDateFrom
).toDate() : null}
        onChange={(e: any) => updateParams?.({ invoiceDateFrom
: e?.toISOString() })}
        onClear={() => updateParams?.({ invoiceDateFrom
: "" })}
      ...
      <DateFiled
        dateFormat="dd-MMM-yyyy"
        id="fromDate"
        placeholderText={t("table-text-delivery-date")}
        value={params?.deliveryDateFrom ? dayjs(params.deliveryDateFrom).toDate() : null}
        onChange={(e: any) => updateParams?.({ deliveryDateFrom: e?.toISOString() })}
        onClear={() => updateParams?.({ deliveryDateFrom: "" })}
        className="w-auto min-w-[242px]"
      />
...
export default function AccountReceivableTableToolbar<
