...
import { statusOptions } from "@/components/container/tan-table/form-utils"
import FilterIcon from "@/components/icons/filter"
import { Button, Checkbox, Select } from "@/components/ui"
import { salesOrderTypes } from "@/data/crm/quotation"
import { SalesOrderQueryOptions } from "@/modules/crm/types/sales-order"
import { dropdownStyles } from "../meeting/meeting-css"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: SalesOrderQueryOptions
  updateParams?: (newParams: Partial<SalesOrderQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  return (
    <>
      <Input
        type="text"
        placeholder={t("table-text-sales-order-no")}
        value={params?.salesOrderNo}
        onClear={() => updateParams?.({ salesOrderNo: "" })}
        onChange={(e) =>
          updateParams?.({ salesOrderNo: e.target.value || "", pageIndex: 1 })
        }
      ...
      <Select
        ...
        value={
          salesOrderTypes.find((option) => option.label === params?.type) ||
          null
        }
      ...
      <Select
        ...
        value={
          statusOptions.find((option) => option.label === params?.status) ||
          null
        }
...
export default function SalesOrderTableToolbar<
