...
import { CurrencyList, CurrencyQueryOptions } from "@/modules/fms/types"
...
export default function CurrencyTable() {
  ...
  const [params, setParams] = useState<CurrencyQueryOptions>({
    search: "",
    pageIndex: DEFAULT_PAGE_INDEX,
    pageSize: DEFAULT_PAGE_SIZE,
  })

  const updateParams = (newParams: Partial<CurrencyQueryOptions>) => {
  ...
  const bulkDeleteCurrency = useBulkDeleteCurrency()

  const { table, setData, columnOrder, handleDragEndColumn, sensors } =
    useTanStackTable<CurrencyList>({
      tableData: data?.data ?? [],
