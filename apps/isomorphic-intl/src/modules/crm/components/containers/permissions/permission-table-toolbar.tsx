...
import { PermissionQueryOptions } from "@/modules/crm/types/permission"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: PermissionQueryOptions
  updateParams?: (newParams: Partial<PermissionQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.name || params?.module || params?.group
  )

  const handleModuleChange = (option: any) => {
    updateParams?.({
      module: option?.label || "",
      pageIndex: 1,
    })
  ...
  return (
    <>
      ...
      <Input
        type="text"
        placeholder={t("table-text-group")}
        value={params?.group}
        onClear={() => updateParams?.({ group: "" })}
        onChange={(e) =>
          updateParams?.({ group: e.target.value || "", pageIndex: 1 })
        }
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
        className="min-w-[242px]"
      ...
      <Select
        ...
        value={
          moduleOptions.find((option) => option.label === params?.module) ||
          null
        }
        placeholder={t("table-text-select-module")}
