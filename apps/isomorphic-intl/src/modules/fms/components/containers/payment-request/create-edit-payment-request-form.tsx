...
export default function CreateEditPaymentRequestForm({
  ...
  const defaultValues: PaymentRequestFormInput = {
    ...
    modeOfPaymentId: 0,
    partyType: requestFor === "HRMS" ? "Employee" : "",
    partyId: requestFor === "HRMS" ? payslipById?.employeeContract?.employeeId : "",
    partyName: requestFor === "HRMS" ? payslipById?.employeeContract?.employee?.firstName + " " + payslipById?.employeeContract?.employee?.lastName : "",
    referenceType: requestFor === "HRMS" ? "Payslip" : "",
    referenceNumber: requestFor === "HRMS" ? payslipById?.serialNumber : "",
    amount: requestFor === "HRMS" ? payslipById?.netPayableSalary! : 0,
    transactionCurrencyId: 0,
    partyAccountCurrencyId: requestFor === "HRMS" ? payslipById?.employeeContract?.company?.currencyId! : 0,
    bankAccountId: 0,
    bankName: "",
    bankAccountNumber: "",
  }
    ...
    <Box>
      <Form<PaymentRequestFormInput>
        ...
        {({ register, control, formState, watch, setValue }) => {
          ...
          // Initialize form with HRMS data
          useEffect(() => {
            if (requestFor === "HRMS" && payslipById) {
              // Set required form values from HRMS data
              setValue("paymentRequestType", "External")
              setValue("companyId", payslipById.employeeContract?.companyId || 0)
              setValue("partyType", "Employee")
              setValue("partyId", payslipById.employeeContract?.employeeId || 0)
              setValue("partyName",
                (payslipById.employeeContract?.employee?.firstName || "") +
                " " +
                (payslipById.employeeContract?.employee?.lastName || ""))
              setValue("referenceType", "Payslip")
              setValue("referenceNumber", payslipById.serialNumber || "")
              setValue("amount", payslipById.netPayableSalary || 0)
              setValue("partyAccountCurrencyId", payslipById.employeeContract?.company?.currencyId || 0)

              // Also set the transaction currency to match the company currency
              if (payslipById.employeeContract?.company?.currencyId) {
                setValue("transactionCurrencyId", payslipById.employeeContract?.company?.currencyId)
              }
            }
          }, [requestFor, payslipById, setValue])
