...
import { useBulkDeleteBudget } from "@/modules/fms/hooks/use-budget"
import { useCompanyList } from "@/modules/fms/hooks/use-company"
import { BudgetQueryOptions, FiscalYearList } from "@/modules/fms/types"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: BudgetQueryOptions
  updateParams?: (newParams: Partial<BudgetQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: budgetAgainstList, isLoading: isBudgetAgainstLoading } = useBudgetAgainstList()
  const { data: fiscalYearList, isLoading: isFiscalYearLoading } = useFiscalYearList()
  const { data: companyList, isLoading: isCompanyLoading } = useCompanyList()
  const isFiltered = Boolean(
    params?.search ||
    params?.companyId ||
    params?.budgetAgainstId ||
    params?.fiscalYearId
  )

  const budgetTypeOptions = useSelectOptions<BudgetAgainstList>(budgetAgainstList?.data, "budgetAgainstName")

  const fiscalYearOptions = useSelectOptions<FiscalYearList>(fiscalYearList?.data, "yearRange")
      ...
      <Select
        ...
        value={
          budgetTypeOptions.find(
            (option) => option.value === params?.budgetAgainstId
          ) || null
        }
      ...
      <Select
        ...
        value={
          fiscalYearOptions.find(
            (option) => option.value === params?.fiscalYearId
          ) || null
        }
...
export default function BudgetTableToolbar<TData extends Record<string, any>>({
  table,
  params,
  updateParams,
