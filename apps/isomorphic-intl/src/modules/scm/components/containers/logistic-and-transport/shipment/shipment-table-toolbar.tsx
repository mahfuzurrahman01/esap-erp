...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: ShipmentQueryOptions
  updateParams?: (newParams: Partial<ShipmentQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { data: shipmentIdList, isLoading: isShipmentIdLoading } =
    useShipmentList({
      pageSize: 1000,
    })

  const isFiltered = Boolean(
    params?.search || params?.carrierName || params?.shipmentId
  )

  const carrierNameOptions = useSelectOptions<Carrier>(
    carrierList?.data,
    "carrierName"
  )

  const shipmentIdOptions = useSelectOptions<Shipment>(
    shipmentIdList?.data,
    "id"
  )

  const handleCarrierNameChange = (option: any) => {
    updateParams?.({
      carrierName: option?.label || "",
      pageIndex: 1,
    })
  }

  const handleShipmentIdChange = (option: any) => {
    updateParams?.({
      shipmentId: option?.label || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      <Select
        ...
        value={
          carrierNameOptions.find(
            (option) => option.label === params?.carrierName
          ) || null
        }
        ...
        placeholder={
          isCarrierLoading
            ? "Loading carrier names..."
            : t("table-text-select-carrier-name")
        }
      ...
      <Select
        ...
        value={
          shipmentIdOptions.find(
            (option) => option.label === params?.shipmentId
          ) || null
        }
        onChange={handleShipmentIdChange}
        isLoading={isShipmentIdLoading}
        isDisabled={isShipmentIdLoading}
        placeholder={
          isShipmentIdLoading
            ? "Loading shipment id..."
            : t("table-text-select-shipment-id")
        }
        className="min-w-[242px]"
      />
