...
import { ProductQueryOptions } from "@/modules/scm/types/inventory/products/products-types"
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { productCategory } = useSCMSharedDataHook([
    "productCategory",
  ])

  const { productCategoryOptions, isLoading: isProductCategoryLoading } =
    productCategory
  ...
  const handleProductCategoryChange = (option: any) => {
    updateParams?.({
      productCategoryName: option?.label || "",
      pageIndex: 1,
    })
  }

  const handleIsFixedAssetChange = (option: any) => {
    updateParams?.({
      isFixedAsset: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      ...
      <Select
        ...
        value={
          productCategoryOptions.find(
            (option: any) => option.label === params?.productCategoryName
          ) || null
        }
        ...
        placeholder={
          isProductCategoryLoading
            ? t("table-text-loading")
            : t("table-text-select-product-category")
        }
        className="min-w-[242px]"
      />
      ...
      <Select
        ...
        value={
          productTypes.find(
            (option: any) => option.value === params?.productType
          ) || null
        }
        ...
        placeholder={
          isProductCategoryLoading
            ? t("table-text-loading")
            : t("table-text-select-product-type")
        }
        className="min-w-[242px]"
      />
      ...
      <Select
        labelClassName="text-title"
        options={isFixedAsset}
        value={
          isFixedAsset.find(
            (option: any) => option.value === params?.isFixedAsset
          ) || null
        }
        onChange={handleIsFixedAssetChange}
        isLoading={isProductCategoryLoading}
        isDisabled={isProductCategoryLoading}
        placeholder={
          isProductCategoryLoading
            ? t("table-text-loading")
            : t("table-text-select-is-fixed-asset")
        }
        className="min-w-[242px]"
      />
      ...
      <Input
        type="text"
        placeholder={t("table-text-product-code")}
        className="min-w-[242px]"
        value={params?.productCode}
        onClear={() => updateParams?.({ productCode: "" })}
        onChange={(e) => updateParams?.({ productCode: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
