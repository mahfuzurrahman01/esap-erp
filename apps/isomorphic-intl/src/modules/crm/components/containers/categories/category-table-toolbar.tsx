...
import { statusOptions } from "@/data/crm/users"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  updateParams?: (newParams: Partial<CategoryQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search || params?.name || params?.parentCategory || params?.status
  )
  ...
  const handleParentCategoryChange = (option: any) => {
    updateParams?.({
      parentCategory: option?.label || "",
      pageIndex: 1,
    })
  }
      ...
      <Select
        ...
        value={
          categoryOptions.find(
            (option) => option.label === params?.parentCategory
          ) || null
        }
