...
import { RiskAssessmentQueryOptions } from "@/modules/scm/types/supplier-relationship/risk-assessment/risk-assessment-types";



import { riskStatusOptions, riskTypeOptions } from "../create-risk-assessment/data-options";
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: RiskAssessmentQueryOptions
  updateParams?: (newParams: Partial<RiskAssessmentQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const handleRiskTypeChange = (option: any) => {
    updateParams?.({
  ...
  const handleRiskStatusChange = (option: any) => {
    updateParams?.({
      riskStatus: option?.value || "",
      pageIndex: 1,
    })
  }
      ...
      <Select
        ...
        value={
          riskTypeOptions.find(
            (option) => option.value === params?.riskType
          ) || null
        }
        onChange={handleRiskTypeChange}
        placeholder={t("table-text-risk-type")}
        className="min-w-[242px]"
      />
      ...
      <Select
        ...
        value={
          riskStatusOptions.find(
            (option) => option.value === params?.riskStatus
          ) || null
        }
        onChange={handleRiskStatusChange}
        placeholder={t("table-text-risk-status")}
        className="min-w-[242px]"
      />
...
export default function RiskAssessmentTableToolbar<
