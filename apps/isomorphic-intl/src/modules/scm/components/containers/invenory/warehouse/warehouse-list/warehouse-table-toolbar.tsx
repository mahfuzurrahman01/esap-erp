...
import { WarehouseQueryOptions } from "@/modules/scm/types/inventory/warehouse/warehouse-types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: WarehouseQueryOptions
  updateParams?: (newParams: Partial<WarehouseQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const { warehouse, warehouseManager } = useSCMSharedDataHook([
    "warehouse",
    "warehouseManager",
  ])
  ...
  const handleManagerNameChange = (option: any) => {
    updateParams?.({
  ...
  return (
    <>
      <Select
        ...
        value={
          warehouseOptions.find(
            (option: any) => option.label === params?.location
          ) || null
        }
        ...
        placeholder={
          isWarehouseLoading
            ? t("table-text-loading-locations")
            : t("table-text-select-location")
        }
      ...
      <Select
        ...
        value={
          warehouseManagerOptions.find(
            (option: any) => option.label === params?.managerName
          ) || null
        }
        ...
        placeholder={
          isWarehouseManagerLoading
            ? t("table-text-loading-manager-names")
            : t("table-text-select-manager-name")
        }
        className="min-w-[242px]"
      />
      ...
      <Input
        type="number"
        className="min-w-[242px]"
        placeholder={t("table-text-capacity")}
        value={params?.capacity}
        onClear={() => updateParams?.({ capacity: "" })}
      ...
      <Input
        type="number"
        className="min-w-[242px]"
        placeholder={t("table-text-in-use-capacity")}
        value={params?.inUseCapacity}
        onClear={() => updateParams?.({ inUseCapacity: "" })}
        onChange={(e) => updateParams?.({ inUseCapacity: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
        clearable={true}
      />
