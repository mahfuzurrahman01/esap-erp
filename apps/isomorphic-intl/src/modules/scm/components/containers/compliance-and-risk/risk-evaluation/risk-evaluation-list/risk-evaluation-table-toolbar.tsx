...
import { RiskEvaluationQueryOptions } from "@/modules/scm/types/compliance-and-risk/risk-evaluation"
...
interface TableToolbarProps<T extends Record<string, any>> {
  ...
  params?: RiskEvaluationQueryOptions
  updateParams?: (newParams: Partial<RiskEvaluationQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const handleRiskImpactChange = (option: any) => {
    updateParams?.({
      riskImpact: option?.value || "",
      pageIndex: 1,
    })
  }

  const handleMitigationStatusChange = (option: any) => {
    updateParams?.({
      mitigationStatus: option?.value || "",
      pageIndex: 1,
    })
  }
  ...
  return (
    <>
      <Select
        ...
        value={
          riskTypeOptions.find((option) => option.value === params?.riskType) ||
          null
        }
      ...
      <Select
        ...
        value={
          riskImpactOptions.find(
            (option) => option.value === params?.riskImpact
          ) || null
        }
        onChange={handleRiskImpactChange}
        placeholder={t("table-text-risk-impact")}
        className="min-w-[242px]"
      />
      ...
      <Select
        ...
        value={
          mitigationStatusOptions.find(
            (option) => option.value === params?.mitigationStatus
          ) || null
        }
        onChange={handleMitigationStatusChange}
        placeholder={t("table-text-mitigation-status")}
        className="min-w-[242px]"
      />
...
export default function RiskEvaluationTableToolbar<
