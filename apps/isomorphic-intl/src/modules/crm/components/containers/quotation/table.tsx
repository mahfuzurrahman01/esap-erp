...
export default function QuotationTable() {
  ...
  const deleteItemBulk = useBulkDeleteQuotation()

  const { params, updateParams } = useQueryParams<QuotationQueryOptions>({
    params: [
      {
        key: "search",
        defaultValue: "",
  ...
  const { data, isLoading, isPending, refetch } = useQuotationList({
    search: debouncedSearchTerm,
    page: params.pageIndex,
    pageSize: params.pageSize,
    quataton: params.quotationNo,
  ...
  const { table, setData, sensors, columnOrder, handleDragEndColumn } =
    useTanStackTable<QuotationList>({
      tableData: data?.data ?? [],
  ...
  const { handleDeleteRow, handleMultipleDelete } =
    useTableDelete<QuotationList>({
      deleteMutation: deleteQuotation.mutate,
