...
import { useBulkDeleteEmployees } from "@/hooks/hrms/employee/use-employee"
...
function FilterElements<T extends Record<string, any>>({
  ...
  const handleJobPositionChange = (option: any) => {
    updateParams?.({
      jobPositionId: option?.value || "",
      pageIndex: 1,
    })
  }
      ...
      <Select
        ...
        value={
          jobPositionOptions.find(
            (option) => option.value === params?.jobPositionId
          ) || null
        }
        onChange={handleJobPositionChange}
        isLoading={isJobPositionLoading}
        isDisabled={isJobPositionLoading}
        placeholder={
          isJobPositionLoading
            ? "Job Position..."
            : t("table-text-select-job-position")
        }
