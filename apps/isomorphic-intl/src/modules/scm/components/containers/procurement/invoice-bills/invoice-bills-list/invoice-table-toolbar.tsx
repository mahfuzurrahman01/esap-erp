...
import { billingStatus } from "@/modules/scm/constants/shared-status-field-option"
import { InvoiceQueryOptions } from "@/modules/scm/types/procurement/invoice/invoice-types"

interface TableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: InvoiceQueryOptions
  updateParams?: (newParams: Partial<InvoiceQueryOptions>) => void
  onClear?: () => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  const isFiltered = Boolean(
    params?.search ||
      ...
      params?.requisitionNo ||
      params?.purchaseOrderNo ||
      params?.invoiceBillNo
  )

  const handleBillingStatusChange = (option: any) => {
    updateParams?.({
  ...
  return (
    <>
      <Input
        ...
        placeholder={t("table-text-requisition-no")}
        value={params?.requisitionNo}
        onClear={() => updateParams?.({ requisitionNo: "" })}
        onChange={(e) => updateParams?.({ requisitionNo: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      <Input
        className="min-w-[242px]"
        placeholder={t("table-text-purchase-order-no")}
        value={params?.purchaseOrderNo}
        onClear={() => updateParams?.({ purchaseOrderNo: "" })}
        onChange={(e) => updateParams?.({ purchaseOrderNo: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      <Input
        className="min-w-[242px]"
        placeholder={t("table-text-invoice-bill-no")}
        value={params?.invoiceBillNo}
        onClear={() => updateParams?.({ invoiceBillNo: "" })}
        onChange={(e) => updateParams?.({ invoiceBillNo: e.target.value })}
        inputClassName="h-10 rounded-lg border-gray-500/20 ring-0 placeholder:text-gray-500 dark:placeholder:text-gray-600"
      />
      <Select
        ...
        value={
          billingStatus.find(
            (option) => option.value === params?.billingStatus
          ) || null
        }
        onChange={handleBillingStatusChange}
        placeholder={t("table-text-status")}
        className="min-w-[242px]"
      />
      {isFiltered && (
        <Button
...
export default function PurchasedOrderTableToolbar<
...
>({ table, params, updateParams }: TableToolbarProps<TData>) {
  ...
  return (
    <div className="flex items-center justify-between px-5">
      ...
      <div className="flex items-center gap-4 grow-0 shrink-0 basis-auto">
        <Button
          {...(isMediumScreen
            ? {
                onClick: () => {
          ...
          variant={"outline"}
          className={cn(
            "h-[36px] rounded-lg border-gray-500/20 pe-3 ps-2.5 text-title dark:text-gray-0 flex flex-grow",
