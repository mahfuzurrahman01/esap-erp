...
import { useMaterialRequirementPlanningApprovalForm } from "./use-material-requirement-planning-approval-form"

type MaterialRequirementPlanningApprovalFormDrawerViewProps = {
  isEditForm?: boolean
  initialData?: MaterialRequirementsPlanApproval
} & (
  | { isEditForm: true; initialData: MaterialRequirementsPlanApproval }
  | { isEditForm?: false; initialData?: MaterialRequirementsPlanApproval }
)

const MaterialRequirementPlanningApprovalFormDrawerView = ({
  isEditForm = false,
  initialData,
}: MaterialRequirementPlanningApprovalFormDrawerViewProps) => {
  const t = useTranslations("form")

  const { isLoading, onSubmit, handleCloseDrawer } =
    useMaterialRequirementPlanningApprovalForm(isEditForm, initialData)
  ...
  return (
    <div className="flex h-full flex-col">
      <DrawerHeader
        heading={
          isEditForm
            ? t(messages.editMaterialRequirementPlanningApproval)
            : t(messages.addNewMaterialRequirementPlanningApproval)
        }
        onClose={handleCloseDrawer}
        headerClassName="mb-0"
      />
      <Form<MaterialRequirementsPlanApproval>
        ...
        {({ register, control, formState: { errors } }) => (
          <>
            <SimpleBar className="h-full">
              <MaterialRequirementPlanningApprovalForm
                register={register}
                errors={errors}
                control={control}
              />
...
export const MaterialRequirementPlanningApprovalForm = ({
  register,
  control,
  errors,
}: any) => {
...
export default MaterialRequirementPlanningApprovalFormDrawerView
