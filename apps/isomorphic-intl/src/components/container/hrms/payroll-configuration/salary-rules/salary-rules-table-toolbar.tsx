...
import { useBulkDeleteSalaryRules } from "@/hooks/hrms/payroll-configuration/use-salary-rules"
import { SalaryRuleQueryOptions } from "@/types/hrms/payroll-configuration/salary-rules.types"

interface SalaryRulesTableToolbarProps<T extends Record<string, any>> {
  table: ReactTableType<T>
  params?: SalaryRuleQueryOptions
  onClear?: () => void
  updateParams?: (newParams: Partial<SalaryRuleQueryOptions>) => void
}
...
function FilterElements<T extends Record<string, any>>({
  ...
  onClear,
}: SalaryRulesTableToolbarProps<T>) {
  const t = useTranslations("table")
  const isFiltered = Boolean(params?.search || params?.salaryCategoryId)
      ...
      {/* <Select
        ...
        value={
          salaryCategoryOptions.find(
            (option) => option.value === params?.salaryCategoryId
          ) || null
        }
        onChange={handleSalaryCategoryChange}
        isLoading={isSalaryCategoryLoading}
        isDisabled={isSalaryCategoryLoading}
        placeholder={
          isSalaryCategoryLoading
            ? "Loading categories..."
            : t("table-text-select-salary-category")
        }
...
export default function SalaryRulesTableToolbar<
  TData extends Record<string, any>,
>({ table, params, updateParams }: SalaryRulesTableToolbarProps<TData>) {
  const t = useTranslations("table")
  ...
  const {
    mutateAsync: bulkDeleteSalaryRules,
    isSuccess: deleteSuccess,
    isPending: isDeletePending,
  } = useBulkDeleteSalaryRules()
  ...
  const bulkDeleteHandle = () => {
    ...
    bulkDeleteSalaryRules(selectedIds)
  }
